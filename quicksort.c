...
@param comparison_counter a pointer to the counter of array comparisons in the algorithm
...
static int partition(int *arr, const int left, const int right,
    ...
    for (int i = left; i < right; i++) {
        ...
        (*comparison_counter)++; // Each iteration of the loop contains one comparison
    }
...
The algorithm takes the first 3 elements and returns the middle one, using a decision tree.
Comparisons of the array elements are counted using comparison_counter.
...
all elements less than the pivot are on the left and the rest are on the right of the pivot.
Then, the algorithms calls itself recursively on each half of the array.
In addition, the algorithm counts each comparison of array elements, using @see comparison_counter
@param arr the array to sort
@param left the index to sort from
@param right the index to sort until
@param use_good_pivot whether to use a pivot which doesn't give the worst case or not
@param comparison_counter a pointer to the counter of the comparisons, which is incremented each comparison
*/
static void quicksort(int *arr, const int left, const int right,
    ...
    if (left < right) {
        ...
        const int pivot_new_index =
            partition(arr, left, right, comparison_counter); // Getting pivot's new index after partition
...
/**
Given an array, count the number of comparisons it takes in order to sort it using quick-sort.
